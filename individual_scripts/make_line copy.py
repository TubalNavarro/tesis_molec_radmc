...
def make_line_image_freq(trans=2):
    ...
    parser.add_argument('-dv', '--dv', default=2, type=float,   #jaquez 0.574826553576917
    ...
    if args.radmc3d:
        subprocess.run('%s image iline  %d widthkms %.5f linenlam %d npix %d sizeau %.1f incl %.1f vkms %f'%(radmc3d, trans ,(args.dv*args.nchan)/2, args.nchan, args.nx, args.sizeau, args.incl, args.v_sys), shell=True) #sizeau=2*xmax
    ...
    im.writeFits(workdir+'radmc_output.fits',fitsheadkeys={'RESTFREQ':sc_center.to(u.Hz).value, 'CTYPE':'FREQ', 'SPECSYS':'LSRK'}, coord='03h10m05s -10d05m30s') #coord of the simutation source 
    ...
    header_=fits.getheader(workdir+'radmc_output.fits')
    header_freq = header_.copy()

    header_freq['EXTEND']= True   
    header_freq['BTYPE']= 'Intensity'   
    header_freq['OBJECT']= 'Sf3d-Radmc3d'   
    header_freq['BUNIT']= 'Jy/pixel'                      
