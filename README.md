...
# Protection tool for RAM saturation on server

* If one or more mcmc runs ar executed on a server with an N_available_cpu's << total_N_threads_mcmc. RAM memory scales quickly and saturates the server. So permanent mcmc_memory_watchdog.sh is included to prevent crashing of the server.
* Three simultaneus mcmc runs with NWalkers=32 and Nthreads=16, with Npix/dimention=130 shouldn't excess 150 Gb at any given moment. If so, or if 1worker>10 Gb, or available_RAM<150 Gb, all mcmc runs are killed.
* to run watchdog for the first time, give permission:
    chmod +x mcmc_memory_watchdog.sh
    chmod +x start_watchdog.sh
    chmod +x stop_watchdog.sh

and then

    ./start_watchdog.sh final 



* to modify memory limits, change this parameters in start_watchdog.sh:
...
to stop (and to apply changes):
    ./stop_watchdog.sh final 
and then 
    ./start_watchdog.sh final

* Please verify watchdog is running before launching mcmc runs, and verify watchdog keeps running after closing ssh session:

    pgrep -afu "$USER" 'mcmc_memory_watchdog\.sh'
    grep 'process_limit=' mcmc_watchdog/production/watchdog.log | tail -1
